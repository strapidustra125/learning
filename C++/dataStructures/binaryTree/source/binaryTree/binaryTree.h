...
public:
    ...
    public:
        ...
        Node(dataType data=dataType(), Node * pLeft=nullptr, Node * pRight=nullptr)
        {
            this->_data = data;
            this->_pLeft = pLeft;
            this->_pRight = pRight;
...
template<typename T>
void BinTree<T>::pushBack(T data)
{
    if(this->_head == nullptr)
    {
        // Если это первый элемент дерева, то это корень

        this->_head = new Node<T>(data);
    }
    else
    {
        // Если элементы уже есть, то формируем 
    }
